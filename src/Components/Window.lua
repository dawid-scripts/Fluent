...
return function(Config)
    ...
    Window.ContainerHolder = New("CanvasGroup", {
        Size = UDim2.new(1, -Config.TabWidth - 32, 1, -106),
        Position = UDim2.fromOffset(Config.TabWidth + 26, 94),
        BackgroundTransparency = 1
    })
    ...
    Window.SelectorPosMotor:onStep(function(value)
        ...
        if lastValue ~= nil then
            Window.SelectorSizeMotor:setGoal(Spring((math.abs(value - lastValue) / (deltaTime * 45)) + 16, { frequency = 6 }))
            lastValue = value
        end
        lastTime = now
    end)
